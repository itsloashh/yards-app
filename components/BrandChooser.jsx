...
export default function BrandChooser({ onChoose }) {
  ...
  return (
    <div className="fixed inset-0 z-[600] flex flex-col bg-stone-950 animate-fade-in">
      {/* subtle top brand */}
      <div className="pt-10 pb-6 text-center">
        <img src="/logo-sign.png" alt="Yard$" className="w-28 h-20 object-contain mx-auto drop-shadow-lg" />
        <p className="text-stone-400 text-sm mt-2 tracking-wide">Choose where you're headed</p>
      </div>

      <div className="flex-1 flex flex-col gap-4 px-5 pb-8 max-w-md mx-auto w-full justify-center">
        {/* Yard$ card */}
        <button
          onClick={() => choose("yards")}
          className="group relative overflow-hidden rounded-3xl p-6 text-left transition-transform active:scale-[0.98] shadow-xl"
        ...
        {/* Yard$ Golf card — premium wood/green feel */}
        <button
          onClick={() => choose("golf")}
          className="group relative overflow-hidden rounded-3xl p-6 text-left transition-transform active:scale-[0.98] shadow-xl wood-panel"
        >
          <div className="absolute inset-0 bg-gradient-to-br from-emerald-900/70 via-emerald-800/40 to-transparent" />
        ...
        <p className="text-center text-stone-500 text-xs mt-2">You can switch anytime from the menu.</p>
      </div>
    </div>
  );
}

...
export default function SaleCard({ sale, delay = 0 }) {
  ...
  const hoursLeft = sale.expiresAt ? Math.max(0, Math.floor((sale.expiresAt - Date.now()) / 3600000)) : null;
  const isEnding = hoursLeft !== null && hoursLeft < 24 && hoursLeft > 0;
  ...
  return (
    <div onClick={() => router.push(`/sale/${sale.id}`)}
      ...
      <div className="relative h-44 bg-stone-200">
        <img src={sale.photos?.[0]} alt="" className="w-full h-full object-cover" loading="lazy" />

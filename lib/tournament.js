...
import { challengeAdjustment } from "@/lib/scrambleRules";
...
export async function setChallengeWin(tag, hole, won) {
  const { data, error } = await supabase.rpc("golf_set_challenge_win", {
    p_bag_tag: tag, p_hole: hole, p_won: !!won,
  });
  if (error) return { ok: false, error: error.message };
  return data?.ok ? { ok: true } : { ok: false, error: data?.error };
}
...
/**
 * Stroke adjustment a team earned on one hole from cards they used there.
 * A card's score_effect (e.g. -1 for the 15-Footer Bonus) applies per use,
 * and the Jackpot Hole doubles every bonus played on it.
 */
export function holeAdjustment(team, hole, cards = [], holes = []) {
  const ledger = team?.power_up_uses || [];
  const jackpot = holes.find((h) => h.hole_number === hole)?.is_jackpot;
  let adj = 0;
  for (const e of ledger) {
    if (e.hole_number !== hole) continue;
    if (e.entry_type === "grant" && (e.source || "manual") === "auto") continue;
    const card = cards.find((c) => c.id === e.power_up_id);
    const eff = card?.score_effect || 0;
    if (!eff) continue;
    adj += jackpot ? eff * 2 : eff;
  }
  // Winning the hole's challenge counts too
  adj += challengeAdjustment(team, hole, holes);
  return adj;
}

export function teamTotals(team, holes, cards = []) {
  const scores = team?.scores || [];
  let strokes = 0, penalties = 0, adjustments = 0, played = 0, parPlayed = 0;
  for (const s of scores) {
    if (s.strokes == null || s.strokes === 0) continue;
    strokes += s.strokes || 0;
    penalties += s.penalties || 0;
    adjustments += holeAdjustment(team, s.hole_number, cards, holes);
    parPlayed += parFor(holes, s.hole_number);
    played++;
  }
  const total = strokes + penalties + adjustments;
  return { strokes, penalties, adjustments, total, played, toPar: total - parPlayed };
}
...
export function rankTeams(teams = [], holes = [], cards = []) {
  return [...teams]
    .map((t) => ({ ...t, totals: teamTotals(t, holes, cards) }))
    .sort((a, b) => {

...
export async function geocodeAddress(address) {
  ...
  if (!q || q.length < 4) return { success: false, lat: null, lng: null, display: "", city: "", region: "" };
  try {
    const r = await fetch(
    ...
    if (!data || !data.length) return { success: false, lat: null, lng: null, display: "", city: "", region: "" };
    const item = data[0];
    const lat = parseFloat(item.lat);
    const lng = parseFloat(item.lon);
    if (isNaN(lat) || isNaN(lng)) return { success: false, lat: null, lng: null, display: "", city: "", region: "" };
    ...
    return { success: false, lat: null, lng: null, display: "", city: "", region: "" };
  }
}

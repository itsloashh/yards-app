...
// ─── ADMIN DATA: reports (moderation queue) ───
export function useAdminReports() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      // Join in the reported sale + reporter for context
      const { data, error } = await supabase
        .from("reports")
        .select(`
          *,
          sale:target_sale_id (id, title, address, photos, user_id),
...
// ─── ADMIN DATA: single user detail (deep view) ───
export function useAdminUserDetail(userId) {
  const [data, setData] = useState({ user: null, sales: [], boosts: [], reports: [] });
  const [loading, setLoading] = useState(true);
  ...
  const reload = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [profileRes, salesRes, boostsRes, reportsRes] = await Promise.all([
        supabase.from("profiles").select("*").eq("id", userId).maybeSingle(),
        supabase.from("sales").select("*").eq("user_id", userId).order("created_at", { ascending: false }),
        supabase.from("boosts").select("*").eq("user_id", userId).order("created_at", { ascending: false }),
        // Reports filed against this user's sales OR the user directly
        supabase.from("reports").select("*, sale:target_sale_id(title)").or(`target_user_id.eq.${userId}`).order("created_at", { ascending: false }),
      ]);
      setData({
      ...
      console.error("[useAdminUserDetail] error:", err);
    } finally {
      setLoading(false);
    }
  }, [userId]);

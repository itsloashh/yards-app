...
export function AppProvider({ children }) {
  ...
  const handleSignUp = async (name, email, password) => {
    try {
      ...
      // If a session was returned, email confirmation is OFF — user is logged in now.
      // If no session, confirmation is ON — user needs to check email.
  ...
  const loadSales = useCallback(async () => {
    try {
      ...
      const { data, error } = await supabase
        .from("sales")
        .select("*, profiles!left(name, bio, phone, avatar_color, rating, sales_posted)")

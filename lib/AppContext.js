...
export function AppProvider({ children }) {
  ...
  // ─── AUTH HANDLERS ───
  // With email confirmation ON: signUp creates the auth user but doesn't sign them in.
  // The profile row is created on first successful login (via ensureProfile in onAuthStateChange).
  // We pass the name through user_metadata so ensureProfile can use it.
  const handleSignUp = async (name, email, password) => {
    try {
      ...
      // Do NOT attempt profile insert here — user isn't confirmed/authenticated yet.
      // Profile will be created on first login via ensureProfile().
  ...
  // Active: happening now or today (date is today/past but not expired)
  // Upcoming: date is more than 24h in the future
  ...
  const getSaleStatus = (s) => {
    ...
    if (s.dateRaw && new Date(s.dateRaw).getTime() > now + dayMs) return "upcoming";
    return "active";
  };

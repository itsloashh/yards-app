...
export function AppProvider({ children }) {
  ...
  // Upload a profile photo (base64) to the avatars bucket, return its public URL.
  const uploadAvatar = async (base64Image) => {
    if (!user || !base64Image) return null;
    try {
      ...
      // Path must start with the user's id folder for the storage RLS policy
      ...
      console.warn("[uploadAvatar] exception:", e);
      return null;
    }
  };
  ...
  const loadSales = useCallback(async () => {
    try {
      ...
      let { data, error } = await supabase
        .from("sales")
        .select("*, profiles!sales_user_id_fkey(name, bio, phone, avatar_color, avatar_url, rating, review_count, sales_posted)")
        .gte("created_at", thirtyDaysAgo)
        .order("created_at", { ascending: false });

      // Fallback: if the embed fails (e.g. a new profile column doesn't exist yet),
      // retry with only the original columns so seller info still loads.
      if (error) {
        console.warn("[loadSales] full embed failed, retrying with base columns:", error.message);
        const retry = await supabase
          .from("sales")
          .select("*, profiles!sales_user_id_fkey(name, bio, phone, avatar_color, sales_posted)")
          .gte("created_at", thirtyDaysAgo)
          .order("created_at", { ascending: false });
        if (!retry.error) {
          data = retry.data;
          ...
          // Last resort: sales only, no profile data
          console.warn("[loadSales] base embed also failed, loading sales only:", retry.error.message);
      ...
      if (data) {
        ...
        setSales(data.map(s => ({
          ...
          saleType: s.sale_type || "yard",
          boostedUntil: s.boosted_until || null,
          boostPackage: s.boost_package || null,
          startTime: s.start_time || "",
          endTime: s.end_time || "",
          seller: s.profiles ? {
            id: s.user_id,
            name: s.profiles.name, bio: s.profiles.bio || "", phone: s.profiles.phone || "",
            avatarColor: s.profiles.avatar_color || "#059669", avatarUrl: s.profiles.avatar_url || "",
            rating: s.profiles.rating || 0, reviewCount: s.profiles.review_count || 0,
            sales: s.profiles.sales_posted || 0,
          } : { name: "Unknown", bio: "", phone: "", avatarColor: "#059669", avatarUrl: "", rating: 0, reviewCount: 0, sales: 0 },
        })));
        setConnOk(true);
      }
      setSalesLoading(false);
  ...
  const handleCreateSale = async (saleData) => {
    ...
    try {
      ...
      const insertData = {
        ...
        sale_type: saleData.saleType || "yard",
  ...
  // Update an existing sale — only the owner can do this (enforced by RLS).
  const handleUpdateSale = async (saleId, saleData) => {
    if (!user) return { error: "Not signed in" };
    try {
      // Handle photos: saleData.photos may contain a mix of existing URLs (http...)
      // and new base64 images that need uploading.
      let photoUrls = [];
      if (saleData.photos?.length) {
        const existingUrls = saleData.photos.filter(p => typeof p === "string" && p.startsWith("http"));
        const newPhotos = saleData.photos.filter(p => typeof p === "string" && p.startsWith("data:"));
        let uploadedUrls = [];
        if (newPhotos.length) {
          try {
            uploadedUrls = await uploadPhotos(newPhotos);
          } catch (e) {
            console.warn("[updateSale] photo upload failed:", e);
          }
        }
        photoUrls = [...existingUrls, ...uploadedUrls];
      }

      // Recompute expiry from the (possibly changed) dates/times
      const expirationDate = saleData.endDateRaw || saleData.dateRaw;
      let expiresAtMs = null;
      if (expirationDate) {
        if (saleData.endTime && /^\d{2}:\d{2}/.test(saleData.endTime)) {
          ...
          const expDate = new Date(`${expirationDate}T00:00`);
          expDate.setHours(eh, em, 0, 0);
          expiresAtMs = expDate.getTime() + 3600000;
        } else {
          const expDate = new Date(`${expirationDate}T00:00`);
          expDate.setDate(expDate.getDate() + 1);
          expiresAtMs = expDate.getTime();
      ...
      // Only update city/region if we have fresh values (don't wipe on edits that didn't re-geocode)
      if (saleData.city) updateData.city = saleData.city;
      if (saleData.region) updateData.region = saleData.region;
      if (saleData.saleType) updateData.sale_type = saleData.saleType;

      const { error } = await supabase.from("sales").update(updateData).eq("id", saleId).eq("user_id", user.id);
      if (error) {
        console.error("[updateSale] failed:", error);
        return { error: error.message || "Couldn't update your sale. Please try again." };
      }
      await loadSales();
      return { error: null };
    } catch (err) {
      console.error("[updateSale] exception:", err);
      return { error: "We're having trouble connecting right now — please try again." };
    }
  };

  // Increment a sale's view count via atomic RPC. Skips the owner's own views.
  const incrementSaleViews = async (saleId, sale) => {
    try {
      // Don't count the owner viewing their own sale
      if (user && sale?.userId === user.id) return;
      await supabase.rpc("increment_sale_views", { sale_id_input: saleId });
    } catch (err) {
      console.warn("[incrementSaleViews] failed (non-critical):", err);
    }
  };

  // Submit a report against a sale or user. Signed-in only.
  const submitReport = async ({ targetType, saleId, targetUserId, reason, details }) => {
    if (!user) { setShowAuth(true); return { error: "Please sign in to report." }; }
    try {
      const row = {
        reporter_id: user.id,
        target_type: targetType,
        target_sale_id: targetType === "sale" ? saleId : null,
  ...
  // ─── SELLER REVIEWS ───
  // Fetch all reviews for a seller (with reviewer name/avatar).
  const fetchSellerReviews = async (sellerId) => {
    try {
      const { data, error } = await supabase
        .from("reviews")
        .select("*, reviewer:reviewer_id(name, avatar_color, avatar_url)")
  ...
  // Get the current user's existing review for a seller (to prefill edit).
  const getMyReviewForSeller = async (sellerId) => {
    if (!user) return null;
    try {
  ...
  // Submit or update a review (one per reviewer per seller, upsert on conflict).
  const submitReview = async ({ sellerId, saleId, rating, comment }) => {
    if (!user) { setShowAuth(true); return { error: "Please sign in to leave a review." }; }
    if (sellerId === user.id) return { error: "You can't review yourself." };
    ...
    try {
      ...
      // Upsert on the (seller_id, reviewer_id) unique constraint
      const { error } = await supabase
        .from("reviews")
        .upsert(row, { onConflict: "seller_id,reviewer_id" });
      if (error) {
  ...
  const deleteMyReview = async (sellerId) => {
    if (!user) return { error: "Not signed in." };
    try {
      const { error } = await supabase.from("reviews").delete().eq("seller_id", sellerId).eq("reviewer_id", user.id);
      if (error) return { error: "Couldn't remove your review." };
      return { error: null };
    } catch {
  ...
  return <AppContext.Provider value={{
    ...
    handleCreateSale, handleDeleteSale, handleUpdateSale, incrementSaleViews, submitReport, loadSales,
    uploadAvatar, fetchSellerReviews, getMyReviewForSeller, submitReview, deleteMyReview,

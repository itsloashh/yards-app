...
export function AppProvider({ children }) {
  ...
  // Active: sale date is today or in the past, but hasn't expired yet
  // Upcoming: sale date is in the future (more than 24h from now)
  // Past: sale has expired (expires_at has passed)
  const now = Date.now();
  const dayMs = 86400000;

  const getSaleStatus = (s) => {
    // If expired, it's past
    if (s.expiresAt && s.expiresAt <= now) return "past";
    // If sale date is more than 24h in the future, it's upcoming
    if (s.dateRaw && new Date(s.dateRaw).getTime() > now + dayMs) return "upcoming";
    // Otherwise it's active (happening now, today, or no specific date)
    return "active";
  };

...
export async function saveGolfGame(game) {
  const row = {
    ...
    rules: game.rules || "",
    header_image: game.header_image || null,   // single banner photo
    rules_images: game.rules_images || [],     // zoomable reference sheets
...
// Back-compat readers: rows created before the header/rules split still keep
// everything in the legacy `images` array.
export function gameHeaderImage(game) {
  return game?.header_image || null;
}
export function gameRulesImages(game) {
  if (game?.rules_images?.length) return game.rules_images;
  return game?.images || [];
}
